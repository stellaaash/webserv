...
void Connection::append_head() {
    assert(_response.status() == RS_EMPTY);

    queue_write(_response.serialize());
    _response.set_status(RS_HEAD);
}
...
void Connection::append_body_chunk() {
    assert(_response.status() == RS_HEAD);

    if (_response.status() == RS_ERROR) {
        queue_write(error_response(_response.code()));
    }
    if (_response.fd() >= 0) {
