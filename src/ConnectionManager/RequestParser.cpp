...
Status_Parsing RequestParser::parse(std::string& read_buffer, size_t& read_index, Request& request) {

    if (request.status() == EMPTY)
        parse_request_line(read_buffer, read_index, request);

    if (request.status() == REQUEST_LINE)
        parse_headers(read_buffer, read_index, request);

    if (request.status() == BODY)
        parse_body(read_buffer, read_index, request);

    return request.status();
}

Status_Parsing RequestParser::parse_request_line(const std::string& read_buffer,
                                                 size_t& read_index,
                                                 Request& request) {
    if (request.status() != EMPTY)
        return request.status();
...
Status_Parsing RequestParser::parse_headers(const std::string& read_buffer,
                                            size_t& read_index,
                                            Request& request) {
    if (request.status() != REQUEST_LINE)
        return request.status();
    ...
    while (true) {
        ...
        if (line_end == read_index) {
            ...
            if (request.has_header("Content-Length")) {
                ...
                size_t len = 0;
                if (!(stream >> len))
                    return request.status(); // TODO : Exception/Error on NaN
		...
		if (values.empty()) {
			...
			for (size_t i = 0; i < values.size(); ++i)
				request.set_header(key, values[i]);
		}
        read_index = line_end + 2; // skip \r\n
    }
}

Status_Parsing RequestParser::parse_body(const std::string& read_buffer,
                                         size_t& read_index,
                                         Request& request) {
    if (request.status() != BODY)
        return request.status();
