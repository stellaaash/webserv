...
ConnectionManager::ConnectionManager() : _epfd(epoll_create(1)) {
    if (_epfd < 0)
        Logger(LOG_ERROR) << "[ConnectionManager] epoll_create: " << std::strerror(errno);
}
...
ConnectionManager::~ConnectionManager() {
    for (std::map<int, IHandler*>::iterator it = _handlers.begin(); it != _handlers.end(); ++it) {
        ...
        if (epoll_ctl(_epfd, EPOLL_CTL_DEL, fd, NULL) != 0)
            Logger(LOG_ERROR) << "[~ConnectionManager] epoll_ctl: " << std::strerror(errno);
        close(fd);
        delete h;
    }
    _handlers.clear();
...
int ConnectionManager::add(IHandler* h) {
    ...
    if (epoll_ctl(_epfd, EPOLL_CTL_ADD, h->fd(), &ev) != 0) {
        Logger(LOG_ERROR) << "[ConnectionManager::add] epoll_ctl: " << std::strerror(errno);
        return 1;
    };
    _handlers[h->fd()] = h;
...
int ConnectionManager::mod(IHandler* h) {
    ...
    if (epoll_ctl(_epfd, EPOLL_CTL_MOD, h->fd(), &ev) != 0) {
        Logger(LOG_ERROR) << "[ConnectionManager::mod] epoll_ctl: " << std::strerror(errno);
        return 1;
    }
    return 0;
}
...
void ConnectionManager::del(IHandler* h) {
    ...
    if (epoll_ctl(_epfd, EPOLL_CTL_DEL, fd, NULL) != 0)
        Logger(LOG_ERROR) << "[ConnectionManager::del] epoll_ctl: " << std::strerror(errno);
    _handlers.erase(it);
...
void ConnectionManager::run() {
    ...
    while (!g_stop) {
        ...
        if (fds < 0) {
            Logger(LOG_ERROR) << "[ConnectionManager::run] epoll_wait: " << std::strerror(errno);
            continue;
        }
